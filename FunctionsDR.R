##This function is to estimate "doubly-robust" model. Based on Functions5SR(2).R
...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  #dpart<<-createDataPartition(data1$treat,p=0.1,list=F)
  #data2<<-data1[dpart,]
  data2<<-data1
  ...
  for(i in 1:Sims){
    ...
    assign(paste0("formula",3), as.formula(paste("Y~",paste(predNames2,collapse="+"),paste("+",paste(interactionVars,collapse="+")))))
    ...
    assign(paste0("mr_orig",i), lm(formula3,data=get(paste0("test_data",i))))
  ...
  formula2<<-formula2
  linearVarsOne<<-linearVars
  interactionVarsOnes<<-interactionVars
  counter = 0
  for (i in 1:Sims){
    if(i >= 1 & i <= Sims) {
      counter = i
  ...
  psmodel<-glm(PSformulaUnWt1, data=test_data3, family=binomial(link="logit"),na.action = na.pass)
  pscore<-as.data.frame(predict(psmodel, newdata=test_data3,type="response"),ncol=1)
  inv<-as.data.frame(1/pscore)
  
  
  
  print(here)
  for (i in 1:Sims){
    assign(paste0("test_data",i),cbind(get(paste0("test_data",i)), inv))
    yp<-paste0("test_data",i)
    tmp3<-get(yp)
    colnames(tmp3)[length(get(paste0("test_data",i)))]<-"inv"
    assign(yp,tmp3)
  }
  
  for (i in 1:Sims){
    assign(paste0("test_data",i),cbind(get(paste0("test_data",i)), pscore))
    yp1<-paste0("test_data",i)
    tmp4<-get(yp1)
    colnames(tmp4)[length(get(paste0("test_data",i)))]<-"pscore"
    assign(yp1,tmp4)
  }
  
  ate<-as.data.frame(ifelse(test_data3$treat==1,1/test_data3$pscore,1/(1-test_data3$pscore)))
  
  for (i in 1:Sims){
    assign(paste0("test_data",i),cbind(get(paste0("test_data",i)), ate))
    yp2<-paste0("test_data",i)
    tmp5<-get(yp2)
    colnames(tmp5)[length(get(paste0("test_data",i)))]<-"ate"
    assign(yp2,tmp5)
  }
  ####################Sensitivity Analysis#################################
  
  sen_cont <<- as.data.frame(subset(test_data3, test_data3$treat==0))
  sen_treat <<- as.data.frame(subset(test_data3, test_data3$treat==1))
  num_cont<<-dim(sen_cont)[1]
  num_treat<<-dim(sen_treat)[1]
  lmSen_cont<<-lm(formula4, data=sen_cont, weights = sen_cont$ate)
  lmSen_treat<<-lm(formula4, data=sen_treat, weights = sen_treat$ate)
  ...
  for(i in 1:Sims){
    assign(paste0("mr_doubler",i), lm(formula3,data=get(paste0("test_data",i)),weights = get(paste0("test_data",i))$ate))
    ##assign(paste0("m.out",i), matchit(formula1,data=get(paste0("test_data",i)), method= "nearest"))
    assign(paste0("modelDoublerSumm",i), summary(get(paste0("mr_doubler",i))))
    assign(paste0("modelDoubleSE", i) , get(paste0("modelDoublerSumm",i))[[4]][[nrow(get(paste0("modelDoublerSumm",i))$coefficients)+2]])
    assign(paste0("design.ate",i),svydesign(ids=~1, weights=ate, data=get(paste0("test_data",i))))
    assign(paste0("survey_model",i),svyglm(formula3, design = get(paste0("design.ate",i))))
    assign(paste0("survey_summ",i),summary(get(paste0("survey_model",i))))
  }
  survey_summThree<<-survey_summ3
  
  View(test_data3)
  formulaThree<<-formula3
  mr_doublerTwo<<-mr_doubler2
  modelDoublerSummOne<<-modelDoublerSumm1
  
  
  
  TFrame<-as.data.frame(rep(0,Sims),ncol=4)
  for (i in 1:Sims){
   TFrame[i,1]<-get(paste0("survey_summ",i))$coefficients[[2]]
    TFrame[i,2]<-get(paste0("survey_summ", i))$coefficients[nrow(get(paste0("survey_summ",i))$coefficients)+2]#SE for doubly robust
    TFrame[i,3]<-get(paste0("modelSumm_orig",i))$coefficients[[2]]
    TFrame[i,4]<-get(paste0("modelSE_orig",i))}
  names(TFrame)<-c("DoublyR","DoublyRSE", "unmatched","unmatchedSE")
  View(TFrame)
  for(i in 1:nrow(TFrame)){
    TFrame$m.bias[i]=2-TFrame$DoublyR[i]
    TFrame$um.bias[i]=2-TFrame$unmatched[i]}
  View(TFrame)
  TFrame<<-TFrame
  range.um.bias<-range(TFrame$um.bias)
  range.m.bias<-range(TFrame$m.bias)
  W<-sprintf("The mean bias of unmatched units is %f with a mean SE of %f, and for matched units is %f with a mean SE of %f", mean(TFrame$um.bias),mean(TFrame$unmatchedSE), mean(TFrame$m.bias), mean(TFrame$DoublyRSE))
  ...
  print(formula2)
  TFrame<<-TFrame
  actual.m<-as.vector(TFrame$DoublyR)
  ...
  print(W2)
  W3<-sprintf("The t-test for the sensitivity analysis for doubly robust analysis is t(%.0f)= %.2f",nu,t_beta)
  print(W3)
}
