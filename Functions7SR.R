##R Script for Multivariate missing part on Level 2 and Level 1 varaibles using van Buuren (pp32-33) procedure
##for creating MAR missing data##
...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  set.seed(2)#ensures same results for random components (e.g., partitioning data, random variable selection, etc.)
  ...
  #varNamesOrdered<<-varNames[c(3:15,1,2)]
  #varNamesOrdered<-varNames[c(3:15,1,2)]
  ...
  ##The following lines will create MAR missing on the Level 2 variable SC048Q01 and the level 1 variables ST013Q01##
  n=nrow(test_data)
  logistic<<-function(x) exp(x)/(1+exp(x))
  logistic<-function(x) exp(x)/(1+exp(x))
  set.seed(80122)
  r2.mar<<-1-rbinom(n, 1, logistic(test_data$ST013Q01))
  r2.mar<-1-rbinom(n, 1, logistic(test_data$ST013Q01))
  r2.mar.bool <<- !!+r2.mar
  r2.mar.bool <- !!+r2.mar
  test_data$r<<-r2.mar.bool
  test_data$r<-r2.mar.bool
  for(i in 1:nrow(test_data)){
    test_data$SC048Q01[i]<<-ifelse(test_data$r[i]=="TRUE",test_data$SC048Q01[i],NA)
  }
  for(i in 1:nrow(test_data)){
    test_data$SC048Q01[i]<-ifelse(test_data$r[i]=="TRUE",test_data$SC048Q01[i],NA)
  }
  print("here again")
  r2.mar2<<-1-rbinom(n, 1, logistic(test_data$ST121Q01))
  r2.mar2<-1-rbinom(n, 1, logistic(test_data$ST121Q01))
  r2.mar.bool2 <<- !!+r2.mar2
  r2.mar.bool2 <- !!+r2.mar2
  test_data$r2<<-r2.mar.bool2
  test_data$r2<-r2.mar.bool2
  for(i in 1:nrow(test_data)){
    test_data$ST034Q01[i]<<-ifelse(test_data$r2[i]=="TRUE",test_data$ST034Q01[i],NA)
  }
  for(i in 1:nrow(test_data)){
    test_data$ST034Q01[i]<-ifelse(test_data$r2[i]=="TRUE",test_data$ST034Q01[i],NA)
  }
  write.table(test_data,file="test_data_missing.csv",row.names = FALSE, col.names = TRUE, sep=",",append=FALSE)
  varNames[1]="SC048Q01"
  print("here again 2")
  ##The following code will use the Joint Model Random Intercepts in van Buuren (2012) to multiply impute (n=5) missing data
  imp_pan<-panImpute(test_data,formula=as.formula(fml),n.burn=500,n.iter=100,m=5,seed=1234)
  imp_pan<<-panImpute(test_data,formula=as.formula(fml),n.burn=500,n.iter=100,m=5,seed=1234)
  impList<-mitmlComplete(imp_pan,print="all")
  impList<<-mitmlComplete(imp_pan,print="all")
  #formulaForImp<-paste("Y~1 +",variablesForJMRI2)
  #formulaForImp<<-paste("Y~1 +",variablesForJMRI2)
  #fit<-with(impList,lmer(formulaForImp))
  #fit<<-with(impList,lmer(formulaForImp))
  #LinModel<-round(summary(pool(fit)))
  #LinModel<<-round(summary(pool(fit)))
  #print(LinModel)
  ...
  mr<-NULL
  mr<<-NULL
  TFrameMeta<<-matrix(0,nrow=5, ncol=4)
  TFrameMeta<<-as.data.frame(TFrameMeta)
    for (k in 1:5){
  ...
  ##Assign colnames to the 'k' PercMeanDiffTables
  assign(paste0("PercMeanDiffTable",k), matrix(rep(0,length(names(m.out2$model$coefficients))*4),nrow=4, ncol=length(names(m.out2$model$coefficients))))
  ypr<-paste0("PercMeanDiffTable",k)
  tmp3<-get(ypr)
  colnames(tmp3)<-names(m.out2$model$coefficients)
  assign(ypr,tmp3)
  
  ##Assign rownames to the 'k' PercMeanDiffTables
  yprt<-paste0("PercMeanDiffTable",k)
  tmp4<-get(yprt)
  rownames(tmp4)<-c("MeanDiff", "eQQMedian", "eQQMean", "eQQMax")
  assign(yprt,tmp4)
  
  
  ##Assign the values to the 'k'PercMeanDiffTables
  
  yprtm<-paste0("PercMeanDiffTable",k)
  tmp5<-get(yprtm)
  tmp5[1,]<-summMatched1
  tmp5[2,]<-summMatched2
  tmp5[3,]<-summMatched3
  ...
  PercMeanDiffTableMeta=matrix(rep(0,length(names(m.out2$model$coefficients))*4*k+4),nrow=4*k+4, ncol=length(names(m.out2$model$coefficients)))
  colnames(PercMeanDiffTableMeta)=names(m.out2$model$coefficients)
  View(PercMeanDiffTableMeta)
  ...
  print(W)
  print(W1)
  TFrameMeta[k,1]=mean(TFrame$um.bias)
  TFrameMeta[k,2]=mean(TFrame$unmatchedSE)
  TFrameMeta[k,3]=mean(TFrame$m.bias)
  TFrameMeta[k,4]=mean(TFrame$matchedSE)
    }
  
  View(PercMeanDiffTable1)
  View(PercMeanDiffTable2)
  View(PercMeanDiffTable3)
  View(TFrameMeta)
  W2<-sprintf("Here are the results using the Joint Model random intercepts (van Buuren, 2012). The mean bias of unmatched units is %f with a SE of %f, and for matched units is %f with a SE of %f for 5 imputed data sets", mean(TFrameMeta$V1),mean(TFrameMeta$V2),mean(TFrameMeta$V3),mean(TFrameMeta$V4))
  print(W2)
  PercMeanDiffTableMeta[1,]=PercMeanDiffTable1[1,]
  PercMeanDiffTableMeta[2,]=PercMeanDiffTable2[1,]
  PercMeanDiffTableMeta[3,]=PercMeanDiffTable3[1,]
  PercMeanDiffTableMeta[4,]=PercMeanDiffTable4[1,]
  PercMeanDiffTableMeta[5,]=PercMeanDiffTable5[1,]
  PercMeanDiffTableMeta[6,]=PercMeanDiffTable1[2,]
  PercMeanDiffTableMeta[7,]=PercMeanDiffTable2[2,]
  PercMeanDiffTableMeta[8,]=PercMeanDiffTable3[2,]
  PercMeanDiffTableMeta[9,]=PercMeanDiffTable4[2,]
  PercMeanDiffTableMeta[10,]=PercMeanDiffTable5[2,]
  PercMeanDiffTableMeta[11,]=PercMeanDiffTable1[3,]
  PercMeanDiffTableMeta[12,]=PercMeanDiffTable2[3,]
  PercMeanDiffTableMeta[13,]=PercMeanDiffTable3[3,]
  PercMeanDiffTableMeta[14,]=PercMeanDiffTable4[3,]
  PercMeanDiffTableMeta[15,]=PercMeanDiffTable5[3,]
  PercMeanDiffTableMeta[16,]=PercMeanDiffTable1[4,]
  PercMeanDiffTableMeta[17,]=PercMeanDiffTable2[4,]
  PercMeanDiffTableMeta[18,]=PercMeanDiffTable3[4,]
  PercMeanDiffTableMeta[19,]=PercMeanDiffTable4[4,]
  PercMeanDiffTableMeta[20,]=PercMeanDiffTable5[4,]
  PercMeanDiffTableMeta[21,]=colMeans(PercMeanDiffTableMeta[c(1:5),])
  PercMeanDiffTableMeta[22,]=colMeans(PercMeanDiffTableMeta[c(6:10),])
  PercMeanDiffTableMeta[23,]=colMeans(PercMeanDiffTableMeta[c(11:15),])
  PercMeanDiffTableMeta[24,]=colMeans(PercMeanDiffTableMeta[c(16:20),])
  
  rownames(PercMeanDiffTableMeta)=c("MeanDiff","MeanDiff","MeanDiff","MeanDiff","MeanDiff",
                                    "eQQMedian","eQQMedian","eQQMedian","eQQMedian","eQQMedian","eQQMean",
                                    "eQQMean","eQQMean","eQQMean","eQQMean","eQQMax","eQQMax","eQQMax",
                                    "eQQMax","eQQMax","Mean_MeanDiff","Mean_eQQMedian","Mean_eQQMean","Mean_eQQMax")
  
  PercMeanDiffTableMeta<<-PercMeanDiffTableMeta
  View(PercMeanDiffTableMeta)
  }
