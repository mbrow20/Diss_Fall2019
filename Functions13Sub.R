##This Functions13.R should always be used with subclassification since it does not 
##partition the data to facilitate analysis; data is already split
##Functions13Sub uses the subclass function in MatchIt package to determine ATE treatment effect. 
##The Zelig package is used for outcome analysis. The outcome is the expected value ("ev) from the 5 
##subclasses with covariate adjustment. This functions uses normalized student weights, similar
##to Functions9
...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  for (i in 1:Sims){
    ...
    assign(paste0("m.out",i), matchit(PSformulaUnWt1,data=get(paste0("test_data",i)), method= "subclass", subclass=3))
    ...
    #assign(paste0("modelSumm",i), coef(get(paste0("z.out",i)))[2])
    #assign(paste0("modelSE", i) , get_se(get(paste0("z.out",i)))[[1]][[2]])
    assign(paste0("modelSumm",i), coef(get(paste0("z.out",i)))[[3]][[1]])
    assign(paste0("modelSE", i) , get_se(get(paste0("z.out",i)))[[1]][[2]])
  }
  ...
  ExpectedValuesTable=matrix(rep(0,Sims*2),nrow=Sims, ncol=2)
  colnames(ExpectedValuesTable)=c("Mean","SD")
  
  modelSummOne<<-modelSumm1
  for (i in 1:Sims){
    ExpectedValuesTable[i,1]=mean(get(paste0("x",i)))
    ExpectedValuesTable[i,2]=sd(get(paste0("x",i)))
    #ExpectedValuesTable[i,1]=get(paste0("modelSumm",i))
    #ExpectedValuesTable[i,2]=get(paste0("modelSE",i))
    
  }
  ExpectedValuesTable1<-as.data.frame(ExpectedValuesTable)
  
  for (i in 1:Sims){
    ExpectedValuesTable1$cover[i]<-ifelse((ExpectedValuesTable1[i,1]+1.96*ExpectedValuesTable1[i,2])>2 & 
                        (ExpectedValuesTable1[i,1]-1.96*ExpectedValuesTable1[i,2])<2,1,0)
  }
  
  View(ExpectedValuesTable)
  ExpectedValuesTable1<<-as.data.frame(ExpectedValuesTable1)
  
  TFrame<-as.data.frame(rep(0,3),ncol=3)
  TFrame[1,1]=mean(ExpectedValuesTable[,1])
  TFrame[1,2]=mean(ExpectedValuesTable[,2])
  TFrame[1,3]=2-TFrame[1,1]
  
  names(TFrame)<-c("MeanExpected","MeanSD","bias")
  
  TFrame<<-TFrame
  actual.m<-as.vector(TFrame$MeanExpected)
  ...
  W<-sprintf("The mean of the treatment effect across all 5 subclasses is %f, the standard deviation is %f, and the bias is %f", TFrame[1,1],TFrame[1,2],TFrame[1,3])
  W1<-sprintf("The mean of coverage across all 5 subclasses is %f percent", (sum(ExpectedValuesTable1$cover)/Sims)*100)
  print(W)
  print(W1)
