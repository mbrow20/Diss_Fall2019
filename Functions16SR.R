#########Coarsened Exact Matching (Iacus, King, Porro, 2004)##########
...
##Functions5SR.R is for the 15 Variable 2 Level Simulation##
...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  for (i in 1:Sims){
    ...
    assign(paste0("m.out",i), matchit(PSformulaUnWt1,data=get(paste0("test_data",i)), method= "cem"))
    assign(paste0("m.data",i), match.data(get(paste0("m.out",i))))
    assign(paste0("linearModel",i), lm(formula2,data=get(paste0("m.data",i)),weights=weights))
    
    assign(paste0("modelSumm",i), summary(get(paste0("linearModel",i))))
    #assign(paste0("modelSE", i) , get_se(get(paste0("m.out",i)))[[1]][[2]])
    ##################################
  ...
  indexVarForSD<<-(length(modelSumm2$coefficients)/4)+2
  View(PercMeanDiffTable)
  
  modelSummTwo<<-modelSumm2
  
  TFrame<-as.data.frame(rep(0,Sims),ncol=4)
  for (i in 1:Sims){
    TFrame[i,1]<-get(paste0("linearModel",i))$coefficients[[2]]
    #TFrame[i,2]<-get(paste0("modelSE",i))#SE for matched
    TFrame[i,2]<-get(paste0("modelSumm",i))$coefficients[[indexVarForSD]]
    TFrame[i,3]<-get(paste0("modelSumm_orig",i))$coefficients[[2]]
    #TFrame[i,4]<-get(paste0("modelSE_orig",i))
    TFrame[i,4]<-get(paste0("modelSumm_orig",i))$coefficients[2,2]
    }
  ...
  W2<-sprintf("The RMSE of the matched units is %f, and for the unmatched units is %f", RootMean.m,RootMean.um)
  print(W)
  print(W1)
  print(W2)
  PSformulaUnWt1<<-PSformulaUnWt1
  #Use MatchIt balance diagnostics
