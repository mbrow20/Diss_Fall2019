##Here is a function to use the SPSS MI files. By the way, just 5 imputations
##took roughly an hour to run.
...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  SPSS_1<-read.csv("SPSSMultImpSim_1.csv",header=TRUE, sep=",")
  SPSS_2<-read.csv("SPSSMultImpSim_2.csv",header=TRUE, sep=",")
  SPSS_3<-read.csv("SPSSMultImpSim_3.csv",header=TRUE, sep=",")
  SPSS_4<-read.csv("SPSSMultImpSim_4.csv",header=TRUE, sep=",")
  SPSS_5<-read.csv("SPSSMultImpSim_5.csv",header=TRUE, sep=",")
  ...
  for (k in 1:5){
    ...
    for(i in 1:Sims){
      ...
      as.data.frame(assign(paste0("test_data",i),cbind(Z,get(paste0("Y",i)), test_data[,v+3])))
    ...
    W<-sprintf("The mean bias of unmatched units is %f with SE of %f, and for matched units is %f with SE of %f", mean(TFrame$um.bias),mean(TFrame$unmatchedSE), mean(TFrame$m.bias), mean(TFrame$matchedSE))
  ...
  View(TFrameMeta)
  W2<-sprintf("Here are the results using SPSS Multiple Imputation (n=5). The mean bias of unmatched units is %f with a SE of %f, and for matched units is %f with a SE of %f for 5 imputed data sets", mean(TFrameMeta$V1),mean(TFrameMeta$V2),mean(TFrameMeta$V3),mean(TFrameMeta$V4))
  print(W2)
